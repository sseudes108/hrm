...
def set_unique_style(header_config: dict):
...
def draw_title(header_config: dict):
    title    = header_config.get("title", "").upper()
    subtitle = header_config.get("subtitle", "").upper()
...
def draw_tools(header_config: dict):
    initial = header_config.get("title", "?")[0].upper()
