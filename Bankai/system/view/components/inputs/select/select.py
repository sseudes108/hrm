...
def draw(
    ...
    if in_card:
        return card.draw(
            card.CardConfig(
                context=context, card_id=f"select_input_card_{context.app_name}_{label}_{input_id}", hover=hover
...
def _draw_component(
    ...
    with st.container(key=f"co_input_select_{app_name}_{input_id}"):
