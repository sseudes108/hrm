...
def draw(
    ...
    return card.draw(
        card.CardConfig(
            context=context,
            card_id=f"multiselect_input_card_{input_id}",
            model="filter",
...
def _draw_component(
    ...
    with st.container(key=scoped_key(context, "co_input_select_multi", input_id)):
        selecteds = st.multiselect(
            label=label, options=options, key=scoped_key(context, "input_select_multi", input_id)
        )
