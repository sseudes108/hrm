...
from athena.config.payload import AthenaPayload, TurnoTemplate
...
def draw_sidebar(context):
    """Renderiza os parâmetros da otimização no slot lateral da aplicação."""
    return card.draw(
        card.CardConfig(
            card_id="capacity_parameters",
            context=context,
            model="base",
            variant="surface",
            padding="normal",
            ...
            has_title=True,
            title="Parâmetros",
            subtitle="Configuração da escala",
            title_case="capitalize",
        ),
        card.CardRenderConfig(content=lambda: _draw_sidebar_component(context)),
    )
    ...
    with files_col:
        ...
        with fl_cols[0]:
            csv_uploaded = file_uploader.draw(
                context=context, label="PERFIL CSV", input_id="ortools_up_csv"
            )
        with fl_cols[1]:
            zip_uploaded = file_uploader.draw(
                context=context, label="LOG ZIP", input_id="ortools_up_zip",
                accepted_types=("zip",),
            )
...
def _draw_sidebar_component(context):
    tma = number_input.draw(
        context=context, label="TMA — SEGUNDOS",
        input_id="ortools_tma_inp", default=215,
    )
    ...
    inicio = text_input.draw(
        context=context, label="INÍCIO", input_id="ortools_ope_ini",
    )
    fim = text_input.draw(
        context=context, label="FIM", input_id="ortools_ope_fim",
    )
    ...
    turnos_selecionados_nomes = multiselect.draw(
        context=context,
        input_id="ortools_turnos_mtslct_inp",
        label="TURNOS",
    ...
    if csv_uploaded:
        df_bruto = pd.read_csv(csv_uploaded)
        df_demand = quebrar_demanda_por_sla(df_bruto, sla if sla > 0 else 30)

    turnos = [
        next(turno for turno in CATALOGO_TURNOS.values() if turno.nome == nome)
        for nome in turnos_selecionados_nomes
    ]
    payload = AthenaPayload(df_demand=df_demand, sla=sla, tma=tma, turnos=turnos)
    payload.disparar_otimizacao = disparar_otimizacao
    return payload
